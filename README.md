...
这是一个可本地运行的产品原型，不是已规模化运营的平台。代码真实包含前端、Node/Vercel Functions、Supabase Auth/REST/Storage 接入、SiliconFlow 模型调用、图像本地特征、7 维匹配、通知与认领流程；默认不带任何真实密钥，离线运行时使用内存示例数据、测试身份、Mock 实名和启发式提取。

- 已实现：邮箱注册/密码登录、6 个前端视图、发布/匹配/通知/认领/归还主流程、隐私模糊、离线示例数据、异常降级。
- 真实 API 代码：SiliconFlow Chat Completions、Supabase REST 与 Storage；只有配置自己的环境变量后才会实际发出调用。
- Mock：实名认证、无 Supabase 时的测试身份与认领/评价/举报持久化、示例机构与代保管点。
...
5. 用户通过邮箱密码建立稳定身份；寻物帖在模拟实名后可联系，招领帖需提交验证答案并经发布者审核后解锁完整信息。
...
| 邮箱密码账号 | Supabase Auth 接入代码 + 离线契约测试 | “密码由 Supabase Auth 管理；生产双账号仍需 Preview/隔离环境复验” |
| 实名 | Mock | “只演示权限门槛，不是权威实名服务商接入” |
...
打开 `http://localhost:4173`。默认模式无需安装依赖、无需数据库、无需 API Key：服务会载入内存示例数据；自然语言提取走启发式规则；图片仍可完成本地特征提取。邮箱注册与登录需要配置 Supabase；离线游客路由只保留给自动化测试，不在用户界面展示。
...
- `LOST_FOUND_SUPABASE_ANON_KEY`（推荐，仅由服务端调用 Auth；可兼容已有服务端 Key）
...
交付包不含真实 API Key。邮箱密码由 Supabase Auth 保存和校验，不写入业务表或浏览器代码；服务端在验证成功后签发兼容现有接口的应用 JWT。Vercel 不再暴露破坏性迁移接口；诊断与种子同步接口限制为管理员；JWT 无显式密钥时使用进程级随机密钥。仍需关注公开图片 URL、Mock 实名、应用 JWT 不能即时感知 Supabase 侧封禁、内存限流与缺少生产级审计日志等风险。

账号接入、环境配置、验证步骤和边界见 [docs/auth/supabase-email-password.md](./docs/auth/supabase-email-password.md)。# 第二轮验收补充（2026-07-12）
