...
- Kimi 语义识别：服务端从用户变量 `silicon_flow_api_key` 读取密钥，调用 Kimi K2.6 识别物品名称、类别、颜色、品牌、可见文字和外观特征。
...
Kimi 语义相似度会比较模型识别出的物品名称、类别、颜色、品牌、可见文字和外观特征。因此当前版本已经包含一定程度的语义级图像识别；同时保留本地低层视觉特征匹配，作为网络或模型不可用时的降级能力。
