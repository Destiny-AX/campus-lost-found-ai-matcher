...
function compareElementPosition(slide, firstIndex, secondIndex) {
  ...
  const intersection =
    intersectionWidth > EPS && intersectionHeight > EPS
      ...
      : null;
  // 当两者互相包含（例如文字框与背景形状完全重合）时，也视为包含关系，
  // 避免把文字故意放在形状内部的情况误判为严重重叠。
  if (aContainsB && bContainsA) {
    const containerIndex =
      boundsA.w * boundsA.h >= boundsB.w * boundsB.h ? firstIndex : secondIndex;
    const containedIndex = containerIndex === firstIndex ? secondIndex : firstIndex;
    return {
      relation: "contained",
      containerIndex,
      containedIndex,
...
const getElementBounds = (obj) => {
  ...
  // pptxgenjs 内部对表格等元素使用 EMU 单位（914400 EMU/英寸），
  // 当数值明显大于英寸范围时进行单位转换，避免误判为越界。
